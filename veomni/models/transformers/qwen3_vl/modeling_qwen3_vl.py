    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if query_states.shape[0] != value_states.shape[0] or query_states.shape[0] != key_states.shape[0]:
            print(f"[DEBUG] Shape mismatch before attention: query={query_states.shape}, key={key_states.shape}, value={value_states.shape}")
            print(f"[DEBUG] cos.shape={cos.shape if cos is not None else None}, sin.shape={sin.shape if sin is not None else None}")
