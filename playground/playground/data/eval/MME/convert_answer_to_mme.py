...
if __name__ == "__main__":
    ...
    GT = get_gt(
        data_path='MME_Benchmark_release'
    )
        ...
        with open(os.path.join(result_dir, f'{category}.txt'), 'w') as fp:
            for file, prompt, answer in cate_tups:
                if 'Answer the question using a single word or phrase.' in prompt:
                    if 'Check carefully to make sure your answer is correct.' in prompt:
                        prompt = prompt.replace(
                            'Answer the question using a single word or phrase. Check carefully to make sure your answer is correct.', '').strip()
                    else:
                        prompt = prompt.replace(
                            'Answer the question using a single word or phrase.', '').strip()
                if "Answer the question using a single word or phrase, both begin with 'Yes' or 'No'." in prompt:
                    prompt = prompt.replace(
                        "Answer the question using a single word or phrase, both begin with 'Yes' or 'No'.", '').strip()
                if 'Answer the following question as if you are seeing the image based on the given caption: \n\n' in prompt:
                    prompt = prompt.split(
                        'Answer the following question as if you are seeing the image based on the given caption: \n\n')[-1]
